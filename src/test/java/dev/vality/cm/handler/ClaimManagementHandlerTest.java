...
public class ClaimManagementHandlerTest {
    ...
    public void setUp() throws URISyntaxException {
        client = new THSpawnClientBuilder()
                .withAddress(new URI("http://localhost:" + serverPort + "/v1/cm"))
    ...
    public void testCreateClaimAndGet() {
        ...
        Assertions.assertEquals(claim.getCreatedAt(), actual.getCreatedAt());
        Assertions.assertEquals(claim, actual);
    }
    ...
    public void testCreateClaimAndUpdate() {
        ...
        Assertions.assertEquals(claim, ServiceUtils.callService(() -> client.getClaim("party_id", claim.getId())));
    ...
    public void testTryingToInvalidChansetWhenCreateClaimAndGet() {
        ...
        assertThrows(InvalidChangeset.class,
    ...
    public void testTryingToGetConflictWhenUpdate() {
        List<Modification> modifications = ServiceUtils.generateModifications(
                conversionWrapperService, 1);
        Claim claim = ServiceUtils.createClaim(client, "party_id", modifications);
        Assertions.assertEquals(claim, ServiceUtils.callService(() -> client.getClaim("party_id", claim.getId())));
        assertThrows(ChangesetConflict.class,
                () -> ServiceUtils.runService(
                        () -> client.updateClaim("party_id", claim.getId(), 0, modifications)));
    }

    @Test
    public void testClaimNotFound() {
        assertThrows(ClaimNotFound.class,
    ...
    public void testGetClaimWithWrongParty() {
        Claim claim = ServiceUtils.createClaim(client, conversionWrapperService, "party_id", 5);
        assertThrows(ClaimNotFound.class,
                () -> ServiceUtils.callService(() -> client.getClaim("wrong_party", claim.getId())));
    }
    ...
    public void testCreateClaimAndAccept() {
        ...
        Assertions.assertEquals(ClaimStatus.pending_acceptance(new ClaimPendingAcceptance()),
    ...
    public void testCreateClaimAndDeny() {
        ...
        Assertions.assertEquals(ClaimStatus.denied(new ClaimDenied().setReason("kek")),
                ServiceUtils.callService(() -> client.getClaim("party_id", claim.getId())).getStatus());
    }
    ...
    public void testCreateRequestReviewAndRequestChanges() {
        ...
        Assertions.assertEquals(ClaimStatus.review(new ClaimReview()),
                ServiceUtils.callService(() -> client.getClaim("party_id", claim.getId())).getStatus());
        ServiceUtils.runService(() -> client.requestClaimChanges("party_id", claim.getId(), 1));
        Assertions.assertEquals(ClaimStatus.pending(new ClaimPending()),
                ServiceUtils.callService(() -> client.getClaim("party_id", claim.getId())).getStatus());
    }
    ...
    public void testSearchClaims() {
        ...
        Assertions.assertEquals(claims.size(),
                ServiceUtils.callService(() -> client.searchClaims(claimSearchQuery)).getResult().size());

        claimSearchQuery.setStatuses(List.of(ClaimStatus.pending(new ClaimPending())));
        Assertions.assertEquals(claims.stream()
                        .filter(claim -> claimSearchQuery.getStatuses().contains(claim.getStatus())).count(),
                ServiceUtils.callService(() -> client.searchClaims(claimSearchQuery)).getResult().size());
    ...
    public void testSearchClaimsByClaimId() {
        ...
        Assertions.assertEquals(claim,
                ServiceUtils.callService(() -> client.searchClaims(claimSearchQuery).getResult().get(0)));
    }
    ...
    public void testSearchClaimsWithContinuation() {
        ...
        do {
            ...
            Assertions.assertEquals(claimSearchQuery.getLimit(), claimSearchResponse.getResult().size());
        ...
        Assertions.assertEquals(
                ServiceUtils.callService(() -> client.searchClaims(claimSearchQuery.setLimit(20))).getResult(),
                searchedClaimList);
    }
    ...
    @Test
    public void setAndGetMetadataError() {
        Claim claim = ServiceUtils.createClaim(client, "party_id",
                ServiceUtils.generateModifications(conversionWrapperService,
                        () -> MockUtil.generateTBaseList(
                                Modification.party_modification(new PartyModification()), 5)));
        Assertions.assertEquals(claim, ServiceUtils.callService(() -> client.getClaim("party_id", claim.getId())));
        try {
            ...
            Assertions.assertEquals(5, ServiceUtils.callService(() ->
    ...
    public void removeMetadataTest() {
        ...
        assertThrows(MetadataKeyNotFound.class,
                () -> ServiceUtils.callService(() -> client.getMetadata("party_id", claim.getId(), "key")));
    }
    ...
    public void softRemoveModificationTest() {
        ...
        Assertions.assertEquals(4, claimWithRemovedMod.getChangeset().size());
    }
    ...
    public void updateBadTypeModificationTest() {
        ...
        assertThrows(ModificationWrongType.class,
                () -> ServiceUtils.runService(
                        () -> client.updateModification("party_id", claim.getId(), claim.getRevision(),
    ...
    public void updateClaimModificationTest() {
        ...
        Assertions.assertEquals(commentModificationUnit.getId(),
                changedModificationUnit.getModification().getClaimModification().getCommentModification().getId());
        assertTrue(changedModificationUnit.getModification().getClaimModification().getCommentModification()
                .getModification().isSetChanged());
        assertNotNull(changedModificationUnit.getChangedAt());
    }
    ...
    public void updateDocClaimModificationTest() {
        ...
        Assertions.assertEquals(
                documentModificationUnit.getId(),
        ...
        assertTrue(
                docModificationUnit.getModification().getClaimModification().getDocumentModification().getModification()
                        .isSetChanged());
        assertNotNull(docModificationUnit.getChangedAt());
    }
    ...
    public void updatePartyModificationTest() {
        ...
        assertEquals(shopLocation.getUrl(),
